...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('vpo_transportadores_cache', function (Blueprint $table) {
            ...
            $table->string('cpf_cnpj', 14)->comment('CPF do motorista ou autônomo (11 dígitos)');
            $table->string('antt_rntrc', 20)->comment('Código RNTRC');
            $table->string('antt_nome', 100)->comment('Nome do motorista/autônomo');
            ...
            // Grupo 2: Veículo (3 campos)
            $table->string('placa', 7)->comment('Placa veículo (formato Mercosul)');
            $table->string('veiculo_tipo', 50)->comment('Tipo do veículo (TOCO, TRUCK, etc)');
            $table->string('veiculo_modelo', 100)->nullable()->comment('Modelo do veículo');

            // Grupo 3: Condutor (4 campos)
            $table->string('condutor_rg', 20)->comment('RG do condutor');
            $table->string('condutor_nome', 100)->comment('Nome completo do condutor');
            $table->char('condutor_sexo', 1)->default('M')->comment('Sexo (M/F)');
            $table->string('condutor_nome_mae', 100)->comment('Nome da mãe');
            $table->date('condutor_data_nascimento')->comment('Data de nascimento');

            // Grupo 4: Endereço (4 campos)
            $table->string('endereco_rua', 150)->comment('Logradouro completo');
            $table->string('endereco_bairro', 100)->comment('Bairro');
            $table->string('endereco_cidade', 100)->comment('Município');
            $table->char('endereco_estado', 2)->comment('UF');

            // Grupo 5: Contato (2 campos)
            $table->string('contato_celular', 11)->comment('Celular (11 dígitos: DDD + número)');
            $table->string('contato_email', 100)->comment('E-mail');
