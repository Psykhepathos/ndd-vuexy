...
class ProgressService
{
    ...
    public function getItinerarioPacote($codPac): array
    {
        try {
            ...
            // Buscar pedidos/entregas (com dados completos para o template)
            $sqlEntregas = "SELECT ped.numseqped as seqent, COALESCE(nf.numnotfis, 0) as nf, cli.codcli, raz.desraz as razcli, est.sigest as uf, mun.desmun, bai.desbai, cli.desend, COALESCE(CAST(nf.numnotfis AS CHAR), '-') as numnot, ped.valtotateped as valnot, ped.pesped as peso, ped.volped as volume FROM PUB.carga car INNER JOIN PUB.pedido ped ON ped.codcar = car.codcar INNER JOIN PUB.cliente cli ON cli.codcli = ped.codcli LEFT JOIN PUB.estado est ON est.codest = cli.codest LEFT JOIN PUB.municipio mun ON mun.codest = cli.codest AND mun.codmun = cli.codmun LEFT JOIN PUB.bairro bai ON bai.codest = cli.codest AND bai.codmun = cli.codmun AND bai.codbai = cli.codbai LEFT JOIN PUB.basecliente bc ON bc.codcli = cli.codcli LEFT JOIN PUB.razao raz ON raz.codraz = bc.codraz LEFT JOIN PUB.notafiscal nf ON nf.codped = ped.codped WHERE car.codpac = $codPac AND ped.valtotateped > 0 ORDER BY ped.numseqped";
