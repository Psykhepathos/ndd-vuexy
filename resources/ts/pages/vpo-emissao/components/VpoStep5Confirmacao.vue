...
const calcularCusto = async () => {
  ...
  try {
    ...
    const response = await apiFetch(`/ndd-cargo/roteirizador`, {
      method: 'POST',
...
const aguardarResultadoRoteirizador = async (guid: string, tentativas = 0) => {
  ...
  try {
    ...
    const response = await apiFetch(`/ndd-cargo/resultado/${guid}`)
    const data = await response.json()
...
const emitirVpo = async () => {
  ...
  try {
    ...
    const response = await apiFetch(`/vpo/emissao/iniciar`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
const consultarResultadoEmissao = async () => {
  ...
  try {
    const response = await apiFetch(`/vpo/emissao/${emissaoUuid.value}`)
    const data = await response.json()
