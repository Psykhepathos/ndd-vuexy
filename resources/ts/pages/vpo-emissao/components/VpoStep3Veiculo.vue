...
const carregarVeiculosCache = async () => {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/veiculos-cache/transportador/${transportador.value.codtrn}`
    )
...
const verificarCache = async (placa: string): Promise<any | null> => {
  try {
    ...
    const response = await apiFetch(`${API_BASE_URL}/api/veiculos-cache/${placaNormalizada}`)
...
const salvarNoCache = async (veiculoData: any, codtrn: number) => {
  try {
    await apiFetch(`${API_BASE_URL}/api/veiculos-cache`, {
      method: 'POST',
...
const carregarVeiculos = async () => {
  ...
  try {
    ...
    if (props.formData.motorista.isEmpresa) {
      const response = await fetch(
        `${API_BASE_URL}/api/transportes/${transportador.value.codtrn}`
      )
      const data = await response.json()
...
const validarPlaca = async () => {
  ...
  try {
    ...
    if (cachedVeiculo && cachedVeiculo.tag && cachedVeiculo.status === 'ATIVO') {
      ...
      const response = await apiFetch(`${API_BASE_URL}/api/semparar/status-veiculo`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
