...
const carregarVeiculosCache = async () => {
  ...
  try {
    const response = await fetch(
      `http://localhost:8002/api/veiculos-cache/transportador/${transportador.value.codtrn}`
    )
...
const verificarCache = async (placa: string): Promise<any | null> => {
  try {
    ...
    const response = await fetch(`http://localhost:8002/api/veiculos-cache/${placaNormalizada}`)
...
const salvarNoCache = async (veiculoData: any, codtrn: number) => {
  try {
    await fetch('http://localhost:8002/api/veiculos-cache', {
      method: 'POST',
